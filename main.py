...
class _mysql_(object):
        ...
        if user and password:
            ...
            try:
                ...
                if connection.is_connected():
                    self.window_conn.destroy()
                    main.homeTab(self, Frame)
                    main.updateMainView(self, Frame)
        else:
            messagebox.showwarning("Warning","Input name and password!")
            return 0
        
        # if database:
        #     # MYSQL SERVER CONNECTION AND CURSOR
        #     try:
        #         global connection
    ...
    def connectToDatabase(self):
        ...
        def connectToDatabaseSQL(database):
            if database:
                try:
                    ...
                    if connection.is_connected():
                        ...
                        main.databaseTab(self, database, 0)
            else:
                messagebox.showwarning("Warning","Select database!")

        def DropDatabase(database, frame):
            if database:
                question_response = messagebox.askyesno(" ", "Delete database "+ str(database) + "?")
                if question_response == 1:           
                    try:
                        cursor.execute('DROP DATABASE `' + str(database) + "`")
                    except Error as e:
                        messagebox.showerror("Error",e)
                    else:
                        messagebox.showinfo("Success","Database deleted!")
                        frame.destroy()
                        _database_.connectToDatabase(self)
                        # main.databaseTab(self, Frame, database, 0)
                else:
                    pass
        ...
        btn_connect_db = Button(window_connect_db, text="Connect", command=lambda:connectToDatabaseSQL(
                                                                                            self.db_listbox.get(ANCHOR)
                                                                                            )
                                )
        btn_connect_db.grid(row=1, column=0, columnspan=2, padx=10, pady=10)

        btn_delete_db = Button(window_connect_db, text="Delete database", command=lambda:DropDatabase(self.db_listbox.get(ANCHOR), window_connect_db))
...
class _database_(object):
    def create(self):
        ...
        self.menu_charsets.bind("<<ComboboxSelected>>", displayCollations)
...
class _table_(object):
    ...
    def create(self, database):
        ...
        btn_generate_cols = Button(self.window_create_table, text="Next", command=lambda:_table_.generateColumns(self, int(number_of_columns.get()), input_tbl_name.get(), database))
        btn_generate_cols.pack()

    def generateColumns(self, rows, tbl_name, database):
        ...
        def createTableSQL(database):
            ...
            if validateColumns():
                ...
                try:
                    ...
                    main.databaseTab(self, database, 1)
            else:
                pass
        ...
        if validate(tbl_name):
            ...
            if (tbl_name,) in tbl_list:
                ...
                Button(sub_frame, text="Create table", command=lambda:createTableSQL(database)).grid(row=rows+1, column=8, columnspan=2, ipadx=30, pady=10)

    def insertRecordsSQL(self, database ,selected_tbl, tbl_dict):
        ...
        print('Frame:', tbl_dict[selected_tbl+'_frame'])

        print(tbl_dict[selected_tbl + '_tbl'])
        query.set("INSERT INTO `" + tbl_dict[selected_tbl + '_tbl'] + "` (")

        # IZIET CAURI VISIEM IEVADES LOGIEM UN PIE VALUES PIEVIENO TIKAI TOS LAUKU NOSAKUMUS, KURI NAV TUKŠI
        for n in range(len(tbl_dict[selected_tbl + '_fields'])):
            # JA ENTRYS NAV TUKŠS
            if tbl_dict[selected_tbl + '_entrys'][tbl_dict[selected_tbl + '_fields'][n] + "_entry"].get(): 
                fields.set(fields.get() + tbl_dict[selected_tbl + '_fields'][n] + ", ")
                values.set(values.get() +  "'" + tbl_dict[selected_tbl + '_entrys'][tbl_dict[selected_tbl + '_fields'][n] + "_entry"].get() + "', ")
        ...
        try:
            ...
            main.tableTab(self, tbl_dict[selected_tbl + '_frame'], database, selected_tbl, '')
            print("DATA INSERTED")

    def DeleteRecordsSQL(self, selected_record, selected_tbl, database, tbl_dict):
        if not selected_record['values']:
            ...
            if q_delete:
                ...
                del_record_query.set("DELETE FROM `" + selected_tbl + "` WHERE `" + str(tbl_dict[selected_tbl+"_fields"][0]) + "` = " + str(selected_record['values'][0]))
                try:
                    cursor.execute("USE " + database)
                    print(del_record_query.get())
                    cursor.execute(del_record_query.get())
                    connection.commit()
                except Error as e:
                    messagebox.showerror("Error", e)
                else:
                ...
                pass
    def renameTable(self, selected_tbl, tab_list, database, tbl_dict):
        # print(selected_tbl['text'])
        def renameTableSQL(new_name, old_name, tab_list, database):
            if new_name:
                query = StringVar()
                query.set("RENAME TABLE " + old_name + " TO " + new_name)
                try:
                    cursor.execute(query.get())
                except Error as e:
                    messagebox.showerror("Error", e)
                    ...
                    message = StringVar()
                    message.set("Table name has been changed from " + old_name + " TO " + new_name)
                    messagebox.showinfo("", message.get() )
                    main.databaseTab(self, database, 1)
                    main.tableTab(self, tbl_dict[selected_tbl + '_frame'], database, selected_tbl, new_name)
            else:
                messagebox.showinfo('', 'Please enter new table name!')
        window_rename_table = Toplevel()
        window_rename_table.geometry("200x120")
        window_rename_table.title("Rename table")

        Label(window_rename_table, text='Enter new name: ').pack()
        entry_new_name = Entry(window_rename_table)
        entry_new_name.pack(pady=10)
        
        btn_rename_table = Button(window_rename_table, text='Rename', command=lambda:renameTableSQL(entry_new_name.get(), selected_tbl, tab_list, database))
        btn_rename_table.pack(pady=10)    

class _relations_(object):

    def create(self, tbl_list, database):
        ...
        # print("SELECT COLUMN_NAME, REFERENCED_COLUMN_NAME FROM information_schema.key_column_usage WHERE REFERENCED_TABLE_SCHEMA = '"+ database +"' AND REFERENCED_TABLE_NAME = '" + tbl_list[1] +"' AND TABLE_NAME = '" + tbl_list[0] + "';")
        cursor.execute("SELECT COLUMN_NAME, REFERENCED_COLUMN_NAME FROM information_schema.key_column_usage WHERE REFERENCED_TABLE_SCHEMA = '"+ str(database) +"' AND REFERENCED_TABLE_NAME = '" + str(tbl_list[1]) +"' AND TABLE_NAME = '" + str(tbl_list[0]) + "';")
        for col in cursor:
            print(col)
        ...
        def createRelationSQL(col_0, col_1, tbl_list):
        ...
        def swapTables(tbl_list):
            ...
            if self.swap == True:
                ...
                Button(self.window_relation_view, text = "Go", command = lambda:createRelationSQL(self.temp_combobox['combobox_1'].get(), self.temp_combobox['combobox_0'].get(), tbl_list)).grid(row=3, column=0, columnspan=2)
            ...
            self.temp_combobox['combobox_0'], self.temp_combobox['combobox_1'] = self.temp_combobox['combobox_1'], self.temp_combobox['combobox_0']
            Button(self.window_relation_view, text = "Go", command = lambda:createRelationSQL(self.temp_combobox['combobox_0'].get(), self.temp_combobox['combobox_1'].get(), tbl_list)).grid(row=3, column=0, columnspan=2)
        ...
        Button(self.window_relation_view, text="<-\n->", command = lambda:swapTables(tbl_list)).grid(row=0, column=0, columnspan=2)
        Button(self.window_relation_view, text = "Go", command = lambda:createRelationSQL(self.temp_combobox['combobox_0'].get(), self.temp_combobox['combobox_1'].get(), tbl_list)).grid(row=3, column=0, columnspan=2)
        ...
        def hideRelationInfo(e):
            info_relation.set("")
        def displayRelationInfo(e):
            temp_key_list = []
        ...
        self.relation_treeview.bind('<ButtonRelease-1>', displayRelationInfo)
        self.relation_treeview.bind('<Leave>', hideRelationInfo)

    def addToListB(self, tbl):
        if tbl != "":
    ...
    def selectTablesView(self, database):
        ...
        Button(self.window_select_tbl, text="Add", command=lambda:_relations_.addToListB(self, self.tbl_listbox_a.get(ANCHOR))).grid(row=2, column=0, pady=10, padx=10)
        Button(self.window_select_tbl, text="Remove", command=lambda:_relations_.RemoveFromListB(self, self.tbl_listbox_b.get(ANCHOR))).grid(row=2, column=2, pady=10, padx=10)
        self.btn_relation_go = Button(self.window_select_tbl, text="Next", command=lambda:_relations_.create(self, self.list_selected_tbl, database))

class _query_(object):

    def queryView(self, database):
        ...
        btn_create_query_view = Button(self.window_query_view, text="New", command=lambda:_query_.create(self, database))
        btn_create_query_view.grid(row=0, column=1, sticky=NE)

        list_query_name = []

        query_table = database + "_querys"
        cursor.execute("SHOW TABLES FROM `" + database + "`")
        ...
        if (query_table,) in tables:
            ...
            Button(self.window_query_view, text='Select', command=lambda:_query_.executeCustomQuery(self, database, query_table, listbox_querys.get(ANCHOR))).grid(row=1, column=0, pady=10)
    ...
    def create(self, database):
        ...
        self.tbl_listbox_a.grid(row=1, column=0, pady=10, padx=10)
        self.tbl_listbox_b.grid(row=1, column=2, pady=10, padx=10)
        Button(frame_top_create_query_view, text="Add", command=lambda:_query_.addToListB(self, self.tbl_listbox_a.get(ANCHOR))).grid(row=2, column=0, pady=10, padx=10)
        Button(frame_top_create_query_view, text="Remove", command=lambda:_query_.RemoveFromListB(self, self.tbl_listbox_b.get(ANCHOR))).grid(row=2, column=2, pady=10, padx=10)
        Button(frame_top_create_query_view, text="Next", command=lambda:_query_.generateEntrys(self, database, self.temp_col_list)).grid(row=3, column=4)
    ...
    def addToListB(self, tbl):
        if tbl != "":
            ...
            self.tbl_listbox_b.insert(END, tbl)
            print("Selected tables: ", self.list_selected_tbl)
            _query_.generateListBoxes(self, self.list_selected_tbl, tbl)
    ...
    def RemoveFromListB(self, tbl):
        if tbl != "":
            self.tbl_listbox_b.delete(ANCHOR)
            _query_.removeListBoxes(self, tbl)
            self.list_selected_tbl.remove(tbl)
            self.tbl_listbox_a.insert(END, tbl)    
            print("Selected tables: ", self.list_selected_tbl)
            if len(self.list_selected_tbl) < 6:
    ...
    def addToColListB(self, tbl_name, col_name):
        if col_name != "":
    ...
    def removeFromColListB(self, tbl_name, col_name):
        if col_name != "":
    ...
    def generateListBoxes(self, tbl_list, tbl_name):
        ...
        self.temp_add_btn['add_' + tbl_name] = Button(self.sub_frame_bottom_create_query_view, text=' ↓ ', command=lambda:_query_.addToColListB(self, tbl_name, self.temp_listbox_a['listbox_' + tbl_name].get(ANCHOR)))
        self.temp_remove_btn['remove_' + tbl_name] = Button(self.sub_frame_bottom_create_query_view, text=' ↑ ', command=lambda:_query_.removeFromColListB(self, tbl_name, self.temp_listbox_b['listbox_' + tbl_name].get(ANCHOR)))
    ...
    def removeListBoxes(self, tbl_name):  
    ...
    def generateEntrys(self, database, cols):

        self.window_create_query_view.destroy()

        def createCustomQuery(query_name, col_count, cols):

            def saveCustomQuery(database ,query_name, query, cols):
                table_name = StringVar()
                table_name.set(database + "_querys")
            ...
            if not query_name:
                ...
                saveCustomQuery(database, query_name, query.get(), cols)
        ...
        # CREATE QUERY BUTTON
        btn_create_query = Button(canvas_query_entrys_bottom_right, text="Create", command=lambda:createCustomQuery(entry_query_name.get(), col_count, cols))
        btn_create_query.pack(side=BOTTOM, anchor=SE)


    def executeCustomQuery(self, db, table, query_name):
        self.window_query_view.destroy()
        ...
        main.queryTab(self, query_name, sql_query[0], sql_query[1])
...
class main(object):
    ...
    # LOGIN POPUP
    def login(self):
        self.window_conn = Toplevel()
        self.window_conn.geometry("300x120")
        self.window_conn.title("Connect")

        # LOGIN INPUTS FIELDS AND LABELS
        # host input
        Label(self.window_conn,text="Host: ").grid(row=0, column=0, pady=5, padx=5)
        self.input_host = Entry(self.window_conn, width=25)
        self.input_host.grid(row=0,column=1, padx=20)
        # username entry
        Label(self.window_conn,text="Username: ").grid(row=1, column=0, pady=5, padx=5)
        self.input_user_name = Entry(self.window_conn, width=25)
        self.input_user_name.grid(row=1,column=1, padx=20)
        # password entry
        Label(self.window_conn,text="Password: ").grid(row=2, column=0, pady=5, padx=5)
        self.input_user_psw = Entry(self.window_conn, width=25, show="•")
        self.input_user_psw.grid(row=2,column=1, padx=20)

        btn_login = Button(self.window_conn, text="Log in (HIDDEN USERNAME)", command=lambda:_mysql_.connect(self, str(self.input_host.get()), str(self.input_user_name.get()), str(self.input_user_psw.get())))
        btn_login.grid(row=3, column=0, columnspan=2, pady=15, padx=10, ipadx=60)    
       
        Label(self.window_conn,text="Username: lietotajs\npass: Highpass77\nhost:192.168.43.233").grid(row=3, column=1, pady=5, padx=5)



    def homeTab(self, Frame):
        # ADDS BUTTONS, LABELS AND INFORMATION BAR TO A HOME TAB IF CONNECTION IS CREATED
        if connection.is_connected():
            # hides login button
            self.btn_home_login.grid_remove()
            # ADD BUTTONS AND LABELS
            # add 'connect to a database' button
            self.btn_connect_to_db = Button(self.tab_home, text="Open database", comman=lambda:_mysql_.connectToDatabase(self))
            self.btn_connect_to_db.grid(row=1, column=0)

            self.lbl_or = Label(self.tab_home, text="OR")
            self.lbl_or.grid(row=2, column=0, pady=30)

            self.btn_create_db = Button(self.tab_home, text="Create database", command=lambda:_database_.create(self))
            self.btn_create_db.grid(row=3, column=0)
            
            self.exit_photo = PhotoImage(file = "img/exit.png") 
            self.btn_disconnect = Button(self.tab_home, image=self.exit_photo, command=lambda:_mysql_.disconnect(self))
    ...
    def databaseTab(self, database, refresh):
        
        def closeTab(tab_name):
        ...
        if not refresh:
            ...
            self.tab_menu.add(self.tab_db, text=database)
            ...
            btn_select_tbl = Button(self.tab_db, text="Select table", command=lambda:self.tableTab(Frame, database, self.tbl_listbox.get(ANCHOR), ''))

            # BUTTON RELATION VIEW
            btn_open_realation_view = Button(self.tab_db, text="Relations", command=lambda:_relations_.selectTablesView(self, database))

            # BUTTON OPEN QUERY VIEW
            btn_open_query_view = Button(self.tab_db, text="Querys", command=lambda:_query_.queryView(self, database))
            ...
            if self.tbl_list:
                ...
                question_response = messagebox.askyesno(" ", "Database "+ str(database) + " is empty\nCreate new table?")

                if question_response == 1:
                    _table_.create(self, database)
                else:
                    closeTab(database)
        else:
            update_listbox()
        ...
        btn_create_tbl = Button(self.tab_db, text="Create table", command=lambda:_table_.create(self, database))
        btn_create_tbl.grid(row=2, column=0, pady=10, padx=10)

        # CLOSE BUTTON
        Button(self.tab_db, text = "✖", command=lambda:closeTab(database)).grid(row=0, column=2, sticky=N)
    
    def tableTab(self, Frame, database, selected_tbl, new_name):
        ...
        def refresh(Frame, fields):
            ...
            if new_name:
                self.tab_menu.tab(self.tab_menu.select(), text=new_name)
                cursor.execute("SELECT * FROM `" + new_name + "`")

                self.tbl_dict[selected_tbl + '_tbl'] = selected_tbl
                self.tbl_dict[selected_tbl + '_fields'] = self.field_name_list
                self.tbl_dict[selected_tbl + '_entrys'] = self.data_entry_dir
                self.tbl_dict[selected_tbl + '_frame'] = self.sub_frame_top
        ...
        if new_name:
            self.tab_list.remove(selected_tbl)
            self.tab_list.append(new_name)
            print('Jaunais ciļņu saraksts -', self.tab_list)
            selected_tbl = new_name
        ...
        if selected_tbl not in self.tab_list:
            ...
            # CLOSE BUTTON
            Button(self.tab_tbl, text = "✖", command=lambda:closeTab(self.tab_menu.tab(self.tab_menu.select())['text'])).pack(side=TOP, anchor=NE)

            # EDIT TABLE NAME BUTTON
            edit_tbl_name_btn = Button(self.tab_tbl, text="Edit table name", command=lambda:_table_.renameTable(self, self.tab_menu.tab(self.tab_menu.select())['text'], self.tab_list, database, self.tbl_dict))
            ...
            for n in range(len(self.field_name_list)):
                ...
                self.data_entry_dir[self.field_name_list[n] + "_entry"] =  Entry(sub_frame_bottom, width=19)
                self.data_entry_dir[self.field_name_list[n] + "_entry"].grid(row=1, column=n, padx=6)
                if n>5:
            ...
            # INSERT BUTTON
            insert_button = Button(self.tab_tbl, text="INSERT", command=lambda:_table_.insertRecordsSQL(self, database , self.tab_menu.tab(self.tab_menu.select())['text'], self.tbl_dict))
            insert_button.pack(side=RIGHT)

            # INSERT BUTTON
            insert_multiple_button = Button(self.tab_tbl, text="INSERT MULTIPLE", command=lambda:_table_.insertRecordsSQL(self, database , self.tab_menu.tab(self.tab_menu.select())['text'], self.tbl_dict))
            insert_multiple_button.pack(side=RIGHT)


            # selected_record =  
            # DELETE BUTTON
            insert_button = Button(self.tab_tbl, text="DELETE", command=lambda:_table_.DeleteRecordsSQL(self,table_tree_view.item(table_tree_view.focus()), self.tab_menu.tab(self.tab_menu.select())['text'], database, self.tbl_dict))
            insert_button.pack(side=RIGHT)

            # VIEW TABLE STRUCTURE BUTTON
            tbl_structure_btn = Button(self.tab_tbl, text="Structure", command=lambda:main.tableStructureTab(self, self.tab_menu.tab(self.tab_menu.select())['text'], database, self.tab_tbl))
            tbl_structure_btn.pack(side=RIGHT)


            print("Selected db: ", database)
            print("Selected table: ", selected_tbl)

        else:
            refresh(Frame, self.field_name_list)
            # refresh(self.sub_frame_top, self.field_name_list)

    def queryTab(self, query_name, sql_query, cols):
        ...
        # CLOSE BUTTON
        Button(self.tab_query, text = "✖", command=lambda:closeTab(query_name)).pack(side=TOP, anchor=NE)
    ...
    def tableStructureTab(self, selected_tbl, database, tbl_tab):

        def closeTab(tab_name):
            self.tableTab(Frame, database, selected_tbl, '')
            self.tab_list.remove(tab_name)
            self.tab_menu.forget(self.tab_menu.select())
            # self.tab_menu.select(tbl_tab)

        tbl_tab.destroy()
        self.tab_list.remove(selected_tbl)

        tab_tbl_struct = Frame(self.tab_menu, width=800, height=450)
        tab_tbl_struct.grid(row=0, column=0)
        tab_tbl_struct.grid_propagate(0)
        tab_tbl_struct.grid_columnconfigure(0, weight=1)

        self.tab_menu.add(tab_tbl_struct, text=selected_tbl+'_struct')

        self.tab_list.append(selected_tbl+'_struct')

        self.tab_menu.select(tab_tbl_struct)

        # CLOSE BUTTON
        Button(tab_tbl_struct, text = "✖", command=lambda:closeTab(self.tab_menu.tab(self.tab_menu.select())['text'])).pack(side=TOP, anchor=NE)
        ...
        tbl_column_list = []
        # DEFINE TREEVIEW
        struct_treeview = ttk.Treeview(tab_tbl_struct)
        # DEFINE TREEVIEW COLUMNS
        struct_treeview['columns'] = ('column_name', 'data_type', 'null', 'default', 'extra')
        
        struct_treeview.column('#0', anchor=CENTER, width=60)
        struct_treeview.column('column_name', anchor=CENTER, width=200)
        struct_treeview.column('data_type', anchor=CENTER, width=100)
        #  DEFINE TREEVIEW HEADINGS
        struct_treeview.heading('#0', text="KEY", anchor=CENTER) 
        struct_treeview.heading('column_name', text=selected_tbl, anchor=CENTER) 
        struct_treeview.heading('data_type', text="type", anchor=CENTER)
        struct_treeview.heading('null', text="NULL", anchor=CENTER)
        struct_treeview.heading('default', text="default", anchor=CENTER)
        struct_treeview.heading('extra', text="extra", anchor=CENTER)
        ...
        cursor.execute("DESCRIBE `"+ selected_tbl + "`")
        for cols in cursor:
            print('description: ', cols)
            struct_treeview.insert(parent='', index='end', text=cols[3], values=(cols[0], cols[1], cols[2], cols[4]))
            tbl_column_list.append(cols[0])

        # CREATE A DROPDOWN LIST    
        self.temp_combobox['combobox'] = ttk.Combobox(tab_tbl_struct, value=tbl_column_list)

        # PLACE TREEVIEWS AND COMBOBOXES ON SCREEN
        struct_treeview.grid(row=1, column=0, padx=10, pady=10)
        self.temp_combobox['combobox'].grid(row=2, column=0, padx=10, pady=10)

        btn_select_tbl = Button(tab_tbl_struct, text="DONE", command=lambda:self.tableTab(Frame, database, selected_tbl, ''))
        btn_select_tbl.grid(row=3, column=0, padx=10, pady=10)
