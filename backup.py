...
class _mysql_(object):
    def connect_sql(self, name, psw):
        ...
        if name!='' and psw!='':
            try:
                global connection
                connection = mysql.connector.connect(host='192.168.43.233',
                                                    user=self.name,
                                                    password=self.psw)
                global cursor
            ...
            if selected_db != '':
                try:
                    ...
                    if connection.is_connected():
                        ...
                        main.update_database_tab(self, selected_db, 0)
...
class _table_(object):
        ...
        def create_tbl_query(selected_db):
            ...
            if validate_cols():
                ...
                try:
                    ...
                    main.update_database_tab(self, selected_db, 1)
            else:
                pass
    ...
    def insert_data(self, selected_db ,selected_tbl, tbl_dict):
        ...
        try:
            ...
            main.update_table_tab(self, tbl_dict[selected_tbl + '_frame'], selected_db, selected_tbl, '')
            print("DATA INSERTED")

    def delete_record(self, selected_record, selected_tbl, selected_db, tbl_dict):
        if not selected_record['values']:
            ...
            if q_delete:
                ...
                pass
    def rename_table(self, selected_tbl, tab_list, selected_db, tbl_dict):
        # print(selected_tbl['text'])
        def rename_table_query(new_name, old_name, tab_list, selected_db):
            if new_name:
                query = StringVar()
                ...
                try:
                    ...
                    main.update_database_tab(self, selected_db, 1)
                    main.update_table_tab(self, tbl_dict[selected_tbl + '_frame'], selected_db, selected_tbl, new_name)
            else:
        ...
        btn_rename_table = Button(window_rename_table, text='Rename', command=lambda:rename_table_query(entry_new_name.get(), selected_tbl, tab_list, selected_db))
...
class main(object):
    ...
    def update_database_tab(self, selected_db, refresh):
    ...
    def update_table_tab(self, Frame, selected_db, selected_tbl, new_name):
        ...
        if selected_tbl not in self.tab_list:
            ...
            # CLOSE BUTTON
            Button(self.tab_tbl, text = "✖", command=lambda:close_tab(self.tab_menu.tab(self.tab_menu.select())['text'])).pack(side=TOP, anchor=NE)

            # EDIT TABLE NAME BUTTON
            edit_tbl_name_btn = Button(self.tab_tbl, text="Edit table name", command=lambda:_table_.rename_table(self, self.tab_menu.tab(self.tab_menu.select())['text'], self.tab_list, selected_db, self.tbl_dict))
            ...
            # INSERT BUTTON
            insert_button = Button(self.tab_tbl, text="INSERT", command=lambda:_table_.insert_data(self, selected_db , self.tab_menu.tab(self.tab_menu.select())['text'], self.tbl_dict))
            insert_button.pack(side=RIGHT)

            # INSERT BUTTON
            insert_multiple_button = Button(self.tab_tbl, text="INSERT MULTIPLE", command=lambda:_table_.insert_data(self, selected_db , self.tab_menu.tab(self.tab_menu.select())['text'], self.tbl_dict))
            insert_multiple_button.pack(side=RIGHT)


            # selected_record =  
            # DELETE BUTTON
            insert_button = Button(self.tab_tbl, text="DELETE", command=lambda:_table_.delete_record(self,table_tree_view.item(table_tree_view.focus()), self.tab_menu.tab(self.tab_menu.select())['text'], selected_db, self.tbl_dict))
            insert_button.pack(side=RIGHT)

            # VIEW TABLE STRUCTURE BUTTON
            tbl_structure_btn = Button(self.tab_tbl, text="Structure", command=lambda:main.tbl_struct(self, self.tab_menu.tab(self.tab_menu.select())['text'], selected_db, self.tab_tbl))
    ...
    def tbl_struct(self, selected_tbl, selected_db, tbl_tab):

        def close_tab(tab_name):
            self.update_table_tab(Frame, selected_db, selected_tbl, '')
            self.tab_list.remove(tab_name)
            self.tab_menu.forget(self.tab_menu.select())
        ...
        # CLOSE BUTTON
        Button(tab_tbl_struct, text = "✖", command=lambda:close_tab(self.tab_menu.tab(self.tab_menu.select())['text'])).pack(side=TOP, anchor=NE)
        ...
        btn_select_tbl = Button(tab_tbl_struct, text="DONE", command=lambda:self.update_table_tab(Frame, selected_db, selected_tbl, ''))
